...
function launch(command, args) {
  ...
  children.push(child);
  child.on('error', (error) => { console.error(`Could not start Queue Lab: ${error.message}`); stop(1); });
  child.on('exit', (code) => { if (!stopping) stop(code ?? 1); });
}
console.log('Starting Queue Lab at http://127.0.0.1:5173 — keep this window open.');
