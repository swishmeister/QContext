...
export default function Home() {
  ...
  useEffect(()=>{if(selected)document.getElementById('match-details')?.scrollIntoView({behavior:window.matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth',block:'start'});},[selected]);
  ...
  return <div className="shell">
    ...
    <main className="workspace">
      ...
      {active&&<section id="match-details" className="details" aria-label="Selected match details"><div className="section-heading"><div><p className="eyebrow">{active.id}</p><h2>{active.champion} · {date(active.startedAt)}</h2></div><Button variant="ghost" onClick={()=>setSelected(null)}>Close details</Button></div><div className="rank-comparison"><TeamRank label="Your team" rank={active.teamRanks?.allies}/><span className="versus">VS</span><TeamRank label="Opponents" rank={active.teamRanks?.enemies}/></div><p className="rank-explainer">Average observed rank · all five players per team, including you. Missing and unranked players are excluded. These are ranks when fetched, not historical match ranks. Each division counts as 100 LP; Master, Grandmaster and Challenger share the Master+ LP scale.</p><DuoPanel key={active.id} match={active} disabled={!online||pending} save={async(players,status)=>{await action('/api/duo',{matchId:active.id,players,status});}}/><div className="team-grid">{[true,false].map(ally=><div key={String(ally)}><h3>{ally?'Your team':'Opponents'}</h3>{active.participants.filter(p=>(p.team===active.team)===ally).map(p=><PlayerCard key={p.puuid} player={p} duos={active.duoPairs??[]} players={active.participants}/>)}</div>)}</div></section>}
...
function DuoPanel({match,disabled,save}:{match:Match;disabled:boolean;save:(players:string[],status:string)=>Promise<void>}) {
  ...
  return <section className="duo-panel" aria-label="Duo queue labels"><div className="duo-heading"><h3>Duo queue</h3><span>Match-specific labels</span></div><p>Possible duos shared a team in at least 3 games within both players’ available prior histories. This is a clue, not proof of a premade. No suggestion does not mean solo queued.</p>
